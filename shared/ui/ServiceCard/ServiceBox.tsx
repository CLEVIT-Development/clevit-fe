  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          index % 2 === 0
            ? `url(${RightBackgroundGradient.src})`
            : `url(${BackgroundGradient.src})`,
        backgroundPosition: index % 2 === 0 ? "right 10px" : "",
      }}
      className={`desktop:mb-0 xs:mb-12 w-full bg-no-repeat desktop:bg-[length:600px_400px] bg-[length:300px_300px] flex justify-between items-center desktop:px-5 lg:px-20 ${index % 2 === 0 ? "flex-row bg-right md:bg-auto" : "flex-row-reverse bg-left"}`}
    >
      <div className="desktop:w-1/2 w-full lg:px-[72px] desktop:py-[60px] md:py-5 md:gap-3 desktop:justify-start desktop:items-start flex flex-col justify-start items-center">
        ...
        <div className="w-full flex desktop:justify-start justify-center desktop:gap-x-2 desktop:pt-8 lg:gap-x-4 desktop:pr-7 md:gap-10 gap-5">
          ...
          <StyledLink className="desktop:text-[20px] text-white text-sm" href="/contact-us">
            {buttonTitle}
          </StyledLink>
        </div>
      </div>
