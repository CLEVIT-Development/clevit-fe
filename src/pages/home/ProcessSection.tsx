...
const ProcessSection = () => {
  return (
    <Section title="Processes We Follow" className="space-y-16">
      <div className="w-full flex desktop:space-y-0 xs:space-x-0 desktop:space-x-[90px] xs:space-y-14 desktop:flex-row xs:flex-col justify-center">
        {processesConstants.map(({ id, title, Icon }, index) => (
          <div key={id} className="relative flex items-center even:self-end">
            ...
            {index + 1 !== processesConstants.length && (
              <Connector
                className="absolute right-0 translate-x-[100%] w-[90px]"
